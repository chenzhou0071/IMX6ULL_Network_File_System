...
static int recv_frame(client_session_t* sess, protocol_frame_t* frame, uint8_t* payload) {
    ...
    /* 先接收帧头 */
    int received = tcp_recv(sess->sockfd, buf, sizeof(protocol_frame_t), g_config->timeout * 1000);
    if (received <= 0) {
    ...
    if (need_len > (uint32_t)received) {
        ...
        int recved = tcp_recv(sess->sockfd, buf + received, to_recv, g_config->timeout * 1000);
        if (recved <= 0) {
...
void build_full_path(char* dest, size_t dest_size, const char* base, const char* relative) {
    /* 如果相对路径已经是完整路径（以 base 开头），直接使用 */
    if (strncmp(relative, base, strlen(base)) == 0) {
        snprintf(dest, dest_size, "%s", relative);
    } else {
        /* 否则拼接 base + relative */
...
void build_upload_path(char* dest, size_t dest_size, const char* root_dir, const char* relative) {
    /* 上传路径：直接使用 root_dir + relative（relative 应该是 uploads/xxx 格式） */
    if (relative[0] == '/') {
        /* 如果 relative 以 /uploads/ 开头，去掉开头的 /uploads/，只用 uploads 部分 */
        if (strncmp(relative, "/uploads/", 9) == 0) {
            snprintf(dest, dest_size, "%s/%s", root_dir, relative + 1);  // 去掉 leading /
        } else {
            /* 其他情况，使用 root_dir 的父目录 */
            const char* lastSlash = strrchr(root_dir, '/');
            if (lastSlash && lastSlash != root_dir) {
                size_t parentLen = lastSlash - root_dir;
                snprintf(dest, dest_size, "%.*s%s", (int)parentLen, root_dir, relative);
            } else {
                snprintf(dest, dest_size, "%s%s", root_dir, relative);
            }
        }
    } else {
        /* 相对路径 */
...
void handle_upload_start(client_session_t* sess, protocol_frame_t* frame, uint8_t* payload) {
    ...
    char full_path[512];
    // 使用 root_dir 而不是 upload_dir，避免双重 uploads 拼接
